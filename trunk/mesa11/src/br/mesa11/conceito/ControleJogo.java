...
public class ControleJogo {
	...
	public boolean verificaDentroCampo(Botao botao) {
		if (mesaPanel == null)
			return true;
		if (mesaPanel.getAreaGolBaixo().intersects(
				botao.getShape(1).getBounds2D())
				|| mesaPanel.getAreaGolCima().intersects(
						botao.getShape(1).getBounds2D())) {
			return true;
		}
		boolean dt = (mesaPanel.getCampoBaixoSemLinhas().intersects(
				botao.getShape(1).getBounds2D())
				|| mesaPanel.getCampoCimaSemLinhas().intersects(
						botao.getShape(1).getBounds2D())
				|| mesaPanel.getLinhaGolBaixo().intersects(
						botao.getShape(1).getBounds2D()) || mesaPanel
				.getLinhaGolCima().intersects(botao.getShape(1).getBounds2D()));
		return dt;
	}
	...
	public boolean verificaMetaEscanteio(Botao botao) {
		if (mesaPanel == null)
			return true;
		if ((mesaPanel.getLinhaGolBaixo().intersects(
				botao.getShape(1).getBounds2D()) || mesaPanel.getLinhaGolCima()
	...
	public boolean efetuaJogada(Point p1, Point p2) {
		...
		for (Iterator iterator = botoes.keySet().iterator(); iterator.hasNext();) {
				...
				if (goleiro.getShape(1).contains(p1)) {
					if (veririficaVez(goleiro)) {
						...
						if (retaGoleiro > (goleiro.getRaio() / 2)) {
							...
							) || (goleiro.getTime().equals(getTimeBaixo()) && (mesaPanel
									.getGrandeAreaBaixo().contains(
											goleiro.getShape(1).getBounds()) || mesaPanel
									.getAreaGolBaixo().intersects(
											goleiro.getShape(1).getBounds()))))
			...
			if (raioPonto <= botao.getRaio()) {
				...
				if (botao.getId() != 0
						&& verificaTemBotao(botao.getCentro(), botao)) {
	...
	public void jogadaCPU() {
		...
		if (!timeJogadaVez.isControladoCPU()) {
			return;
		}
		...
		Point metaCimaDireita = mesaPanel.getPequenaAreaCima().getLocation();
		metaCimaDireita.y += mesaPanel.getPequenaAreaCima().getHeight()
				- bola.getDiamentro();
		Point metaCimaEsquerda = new Point(metaCimaDireita.x, metaCimaDireita.y);
		metaCimaDireita.x += mesaPanel.getPequenaAreaCima().getWidth()
				- bola.getDiamentro();

		Point metaBaixoDireita = mesaPanel.getPequenaAreaBaixo().getLocation();
		Point metaBaixoEsquerda = new Point(metaBaixoDireita.x,
				metaBaixoDireita.y);
		metaBaixoDireita.x += mesaPanel.getPequenaAreaBaixo().getWidth()
				- bola.getDiamentro();
		boolean meta = false;
		if (GeoUtil.distaciaEntrePontos(bola.getCentro(), metaCimaDireita) < bola
				.getDiamentro()
		...
		for (int i = 0; i < 50; i++) {
			if (ConstantesMesa11.CAMPO_CIMA.equals(btnPrximo.getTime()
				...
				gol = new Point(Util.intervalo(mesaPanel.getCampoCima().x,
						...
						.intervalo(mesaPanel.getCampoCima().y, mesaPanel
								.getCampoCima().y
								+ mesaPanel.getCampoCima().height));
			}
	...
	private Point obterTrajetoriaCPUGdAreaOposta(Botao btnPrximo) {
		...
		if (GeoUtil.distaciaEntrePontos(bola.getCentro(), gol) < MesaPanel.ALTURA_MESA / 2) {
	...
	private Botao obterBtnProximoAntesLinhaBola(List botoesTimeVez,
		...
		Logger.logar("obterBtnProximoAntesLinhaBola " + livre + "  btn "
				+ btnPrximo);
		return btnPrximo;
	}
	...
	private boolean validaCaimho(Botao ori, Point gol) {
		...
		for (int j = 0; j < linha.size(); j += 5) {
			for (Iterator iterator = botoes.keySet().iterator(); iterator
				...
				if (botaoAnalisado.getId() != 0 && !ori.equals(botaoAnalisado)
						&& botaoAnalisado.getShape(1).contains(pt)
						&& !mesaPanel.getHasteDireitaGolBaixo().contains(pt)
						&& !mesaPanel.getHasteDireitaGolCima().contains(pt)
						&& !mesaPanel.getHasteEsquerdaGolBaixo().contains(pt)
						&& !mesaPanel.getHasteEsquerdaGolCima().contains(pt)) {
					caminhoGol = false;
					break;
				}
			}
